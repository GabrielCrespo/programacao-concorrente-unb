...
sem_t sem_cadeiras, acordar_barbeiro, cadeira_barbeiro, finalizar_corte;
...
void *f_barbeiro(void *v)
{
  ...
  while (1)
  {
    sem_wait(&acordar_barbeiro); //...Esperar/dormindo algum cliente sentar na cadeira do barbeiro (e acordar o barbeiro)

    sleep(1); //Cortar o cabelo do cliente
    printf("Barbeiro cortou o cabelo de um cliente\n");

    sem_post(&finalizar_corte); //...Liberar/desbloquear o cliente
  }
  pthread_exit(0);
}
...
void *f_cliente(void *v)
{
  ...
  { //conseguiu pegar uma cadeira de espera
    ...
    sem_wait(&cadeira_barbeiro); //... pegar/sentar a cadeira do barbeiro
    sem_post(&sem_cadeiras);     //... liberar a sua cadeira de espera
    sem_post(&acordar_barbeiro); //... acordar o barbeiro para cortar seu cabelo
    sem_wait(&finalizar_corte);  //... aguardar o corte do seu cabelo
    sem_post(&cadeira_barbeiro); //... liberar a cadeira do barbeiro
...
int main()
{
  ...
  sem_init(&acordar_barbeiro, 0, 0);
  sem_init(&finalizar_corte, 0, 0);
